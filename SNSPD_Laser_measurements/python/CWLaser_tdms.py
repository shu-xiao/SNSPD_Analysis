...
def SingleTDMS_CW_analysis(in_filename):
    with TdmsFile.open(in_filename) as tdms_file:
        ...
        hist_counts  = plot_histo(counts      , 10, 0, np.max(counts), 'counts', f'Photon Counts / {timeWindow}s', f'{plotDir}/counts.png')
        hist_range   = plot_histo(pulseRanges , 10, 0.02, np.max(pulseRanges), 'signal_range', 'Voltage [V]', f'{plotDir}/signal_range.png')
