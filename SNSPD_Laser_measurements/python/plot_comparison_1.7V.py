...
info = r'$T=4.6K,\quad V_{Bias}=1.7V,\quad 532nm\, pulsed\, laser$'
...
# x_fit = np.linspace(min(sorted_names), 20, 100)
...
# plt.errorbar(sorted_names, sorted_range_mean_fits, yerr=sorted_range_mean_fit_errors, fmt='o', label='Data')
# plt.plot(x_fit, y_fit, 'r-', label='Linear Fit')
# ax_zoom.set_xlim(0, 20)  # Adjust the x-axis limits as desired
# ax_zoom.set_ylim(0, max(y_fit) * 1.1)  # Adjust the y-axis limits as desired
# ax_zoom.set_title('Zoomed-in Plot for x < 20')
# ax_zoom.grid(True)
# ax_zoom.legend()
...
plt.xticks(fontsize=13)
plt.yticks(fontsize=13)
plt.ylim(0, 0.08)  # Set x-axis limits
plt.grid(1)
plt.text(max(sorted_names), 0.083, info, fontsize=13, horizontalalignment='right')
plt.tight_layout()
...
plt.yticks(fontsize=13)
plt.grid(1)
plt.text(max(sorted_names), 0.04, info, fontsize=13, horizontalalignment='right')
plt.tight_layout()
...
plt.xticks(fontsize=13)
plt.yticks(fontsize=13)
plt.ylim(0, 0.2)  # Set x-axis limits
plt.grid(1)
...
plt.text(max(sorted_names), 0.21, info, fontsize=13, horizontalalignment='right')
plt.tight_layout()
# Specify the coordinates and dimensions of the zoomed-in plot within the main plot
...
plt.xticks(fontsize=13)
plt.yticks(fontsize=13)
plt.text(max(sorted_names), max(sorted_sdes)*1.1, info, fontsize=13, horizontalalignment='right')
