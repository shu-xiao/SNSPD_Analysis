...
def Get_df_Arrival(df, chName, value):
    ...
    print("Time of Arrival: {} {} ".format(idx,df.loc[idx,'Time']))
...
def Get_df_RiseFall_Range(df, chName, value):
    ...
    if ( value > 0 ): print("Edge Start: {}, Edge End: {}, Rise Time: {}".format(start_index,end_index,RiseFall_time))
    if ( value < 0 ): print("Edge Start: {}, Edge End: {}, Fall Time: {}".format(start_index,end_index,RiseFall_time))
    ...
    for iroot, root in enumerate(roots):
        if ( roots[iroot-1] > rangeMin and root < rangeMax and root > roots[iroot-1] ):
            ...
            if (Rise_Fall == 'Rise' and inputfunc(root) - inputfunc(roots[iroot-1]) > range_threshold):
                ...
                if (debug): print("Rise Turning Point: ({:.4f},{:.4f}) ({:.4f},{:.4f}) ({:.4f})".format(p_pedestals[-1].x,p_pedestals[-1].y,p_peaks[-1].x,p_peaks[-1].y,p_ranges[-1]))
...
def Get_indices_overThreshold(df, chName, threshold):
    ...
    print ("Sample points with value above threshold: {}".format(indices))
