...
if __name__ == "__main__":
    ...
    for in_filename in args.in_filenames:
        ...
        # ########################################
        npeaks = 5
        f1 = ROOT.TF1("f1", fpeaks, 0.004, 0.01,12)
        f1.SetLineWidth(3)
        f1.SetNpx(10000)
        ...
        f1.SetParameter(1, 0.005) # first peak mean
        ...
        f1.SetParLimits(1,  0.004,0.006) # first peak mean
        f1.SetParLimits(2,  0, 1000) # norm
        f1.SetParLimits(3,  0.00015, 0.0004) # width
        f1.SetParLimits(4,  0, 1000) # norm
        f1.SetParLimits(5,  0.00015, 0.0004) # width
        f1.SetParLimits(6,  0, 1000) # norm
        f1.SetParLimits(7,  0.00015, 0.0004) # width
        f1.SetParLimits(8,  0, 1000) # norm
        f1.SetParLimits(9,  0.00015, 0.0004) # width
        f1.SetParLimits(10, 0, 1000) # norm
        f1.SetParLimits(11, 0.00015, 0.0004) # width
        ...
        info=ROOT.TLatex(0.15,0.9, "4.6K,   1#muW,   532nm pulsed laser,   Binning = 0.5mV / 2^{8} (range/8bits)");
        info.SetNDC(1);
        ...
        altpoissonfit = ROOT.TF1("f1","[0]*TMath::Power(([1]/[2]),(x/[2]))*(TMath::Exp(-([1]/[2])))/TMath::Gamma((x/[2])+1.)", 3, 19) # Alternative poisson from here https://root-forum.cern.ch/t/fitting-a-poisson-distribution-to-a-histogram/12078
        altpoissonfit.SetParameter(0, 100)
        altpoissonfit.SetParameter(1, 10)
        altpoissonfit.SetParameter(2, 0.9)

        altpoissonfit.SetParLimits(1, 4, 12)
