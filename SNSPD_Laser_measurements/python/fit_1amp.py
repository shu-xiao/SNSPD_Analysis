...
if __name__ == "__main__":
    ...
    for in_filename in args.in_filenames:
        ...
        # ########################################
        npeaks = 5
        f1 = ROOT.TF1("f1", fpeaks, 0.006, 0.012,12)
        f1.SetLineWidth(3)
        f1.SetNpx(10000)
        ...
        f1.SetParameter(1, 0.007) # first peak mean
        ...
        f1.SetParLimits(1,  0.007,0.009) # first peak mean
        ...
        for ipeak in range(npeaks):
            ...
            f[ipeak].Draw("same")
            leg.AddEntry(f[ipeak],"%d photons"%(ipeak+6),"l")
            # Append to array
            norms.append(norm)
            means.append(mean)
            sigmas.append(sigma)
            peakIndex.append(ipeak+7)
        ...
        info=ROOT.TLatex(0.15,0.9, "4.6K,   2#muW,   532nm pulsed laser,   Binning = 0.5mV / 2^{8} (range/8bits)");
        info.SetNDC(1);
        ...
        altpoissonfit.SetParLimits(1, 7, 12)
