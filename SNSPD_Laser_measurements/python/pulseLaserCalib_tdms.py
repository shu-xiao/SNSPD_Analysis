...
def SingleTDMS_analysis(in_filename):
    with TdmsFile.open(in_filename) as tdms_file:
        ...
        for event, chunk in enumerate(tdms_file.data_chunks()):
            ...
            for ipulse, (ch2_turning_pedestal, ch2_turning_peak) in enumerate(zip(ch2_turning_pedestals, ch2_turning_peaks)):
                ...
                # if (prePulse_range[-1] < 0.005 or prePulse_stdev[-1] < 0.002 or postPulse_range[-1] < 0.005 or postPulse_stdev[-1] < 0.002):
                    ...
                    # if (ch1_pulse_spline_range > 0.003 and ch1_pulse_diff_range > 0.002):
        ...
        Sideband_results['range']    = plot_2histo(prePulse_range, postPulse_range, 100, 0, 0.1, 'prePulse', 'postPulse', 'Sideband range', f'{plotDir}/sideband_range.png')
        ...
        # root histograms
        plot_histo_root(prePulse_mean, 50, -0.01, 0.01, 'prePulse', 'Voltage [V]', 'PrePulse mean', f'{plotDir}/sideband_prepulse_mean.png')
        plot_histo_root(postPulse_mean, 50, -0.01, 0.01, 'postPulse', 'Voltage [V]', 'PostPulse mean', f'{plotDir}/sideband_postpulse_mean.png')
        plot_histo_root(prePulse_stdev, 50, 0, 0.05, 'prePulse', 'Voltage [V]', 'PrePulse stdev', f'{plotDir}/sideband_prepulse_stdev.png')
        plot_histo_root(postPulse_stdev, 50, 0, 0.05, 'postPulse', 'Voltage [V]', 'PostPulse stdev', f'{plotDir}/sideband_postpulse_stdev.png')
        plot_histo_root(prePulse_range, 100, 0, 0.1, 'prePulse', 'Voltage [V]', 'PrePulse range', f'{plotDir}/sideband_prepulse_range.png')
        plot_histo_root(postPulse_range, 100, 0, 0.1, 'postPulse', 'Voltage [V]', 'PostPulse range', f'{plotDir}/sideband_postpulse_range.png')
        ...
        hist_amplitudes        = plot_histo_root(ch1_pulse_amplitudes       , 256, 0,   vertical_range, 'signal_amplitude', 'Voltage [V]', 'Pulse amplitude',f'{plotDir}/signal_amplitude_root.png')
        hist_range             = plot_histo_root(pulseRanges                , 256, 0,   vertical_range, 'signal_range', 'Voltage [V]', 'Pulse range',f'{plotDir}/signal_range_root.png')
        hist_range_0p1mVbinned = plot_histo_root(pulseRanges                , 300,   0,   0.3,              'signal_range_0p1mVbinned', 'Voltage [V]', 'Pulse range',f'{plotDir}/signal_range_root_0p1mVbinned.png')
        ...
        # arrivalT_mean, arrivalT_meanE, arrivalT_std, arrivalT_stdE = fit_histo_gaus(hist_arrivalTs, 219.5, 220.5, 'fit_signal_arrivalT', 'Time [index]', 'Pulse arrival time fit', f'{plotDir}/signal_arrivalT_fit.png')
