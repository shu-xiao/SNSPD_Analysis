...
for infile in args.in_filenames:
    ...
    baseName= infile.rsplit("/",1)[1].split(".")[0]
    ...
    g_IV[baseName].SetTitle("%s" % infile.split("/")[1])
    g_IV[baseName].GetXaxis().SetTitle("Current (A)")
    g_IV[baseName].GetYaxis().SetTitle("Voltage (V)")
    g_IV[baseName].SetMarkerColor(2)
    g_IV[baseName].Draw("APE")
    # g_IV[baseName].GetYaxis().SetRangeUser(-1., 0.2)

    c1.Print("plots/IV_%s.png" % infile.split("/")[1])

c1.SetLogy()
leg_multi = ROOT.TLegend(0.8, 0.15, 0.88, 0.42)
